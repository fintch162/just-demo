...
var venueTableEditable = function () {
        ...
        init: function () {
          var oTable;
            ...
            function editRow(oTable, nRow) {
                ...
                var aData = oTable.fnGetData(nRow);
                var venue_id = $(aData[1]).attr('id');
                var jqTds = $('>td', nRow);
                oTableForEdit = oTable;
                nRowForEdit = nRow;
               if (typeof(venue_id) != "undefined")
                 {
                  jqTds[0].innerHTML = '<input type="text" id="venue_title'+venue_id+'" class="form-control input-xlarge" value="' + aData[0] + '">';
                  jqTds[1].innerHTML = '<a class="edit" href="javascript:void(0);"  onclick="save_venue('+venue_id+');">Save</a>';
                  jqTds[2].innerHTML = '<a class="cancel" href="">Cancel</a>';
                 }
                else {
              jqTds[0].innerHTML = '<input type="text" id="venue_title" class="form-control input-xlarge" value="' + aData[0] + '">';
                  jqTds[1].innerHTML = '<a class="edit" href="javascript:void(0);"  onclick="save_new_venue();">Save</a>';
                  jqTds[2].innerHTML = '<a class="cancel" href="">Cancel</a>';
                }
    ...
    function save_venue (p_venue_id){
        var venue_title = $("#venue_title"+p_venue_id).val();
        if(venue_title == "") {
            $("#venue_title").css({"border-color":"red"});
            return false;
        } else {
            var url = "/manage/venues/"+ p_venue_id;
          
            $.ajax({
              url: url,
              type: 'put',
              data: {title: venue_title},
            })
            .done(function(data) {
              nEditing = null;
                saveRow(oTableForEdit,nRowForEdit);
